...
import { Sun, Wind, BatteryCharging, House, AlertTriangle, Cpu, TrendingUp, CheckCircle, FileText, Download, MessageSquare, Mail, TestTube2, BookOpen, History, Bell } from 'lucide-react';
...
// --- View Components (Dashboard, Reports, and NEW Alerts View) ---
...
const ConsumptionPieChart = ({ totalConsumption }) => { const categories = [ { name: 'HVAC', value: 0.40 }, { name: 'Lighting', value: 0.15 }, { name: 'Appliances', value: 0.25 }, { name: 'EV Charging', value: 0.12 }, { name: 'Other', value: 0.08 } ]; const pieData = categories.map(cat => ({ name: cat.name, value: parseFloat((totalConsumption * cat.value).toFixed(2)) })); const COLORS = ['#3b82f6', '#16a34a', '#f59e0b', '#06b6d4', '#6366f1']; return (<ResponsiveContainer width="100%" height={350}><PieChart><Pie data={pieData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} fill="#8884d8" label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}><>{pieData.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />))}</></Pie><Tooltip formatter={(value) => `${value} kW`} /><Legend /></PieChart></ResponsiveContainer>); };
...
// --- NEW: Alerts View Component ---
const AlertsView = ({ alerts }) => {
    const severityConfig = { CRITICAL: { iconClass: 'text-red-600', bgClass: 'bg-red-50', borderClass: 'border-red-500' }, WARNING: { iconClass: 'text-yellow-600', bgClass: 'bg-yellow-50', borderClass: 'border-yellow-500' } };
    const defaultConfig = { iconClass: 'text-gray-500', bgClass: 'bg-gray-50', borderClass: 'border-gray-500' };
    ...
    return (
        <div className="space-y-4">
            {alerts.length > 0 ? alerts.map((alert, index) => {
                ...
                return (
                    <div key={index} className={`flex items-start p-4 rounded-lg shadow-sm border-l-4 ${config.bgClass} ${config.borderClass}`}>
                        <AlertTriangle size={24} className={`mr-4 mt-1 flex-shrink-0 ${config.iconClass}`} />
                        <div>
                            <p className={`font-semibold text-lg ${config.iconClass}`}>{alert.type}</p>
            ...
            }) : <div className="bg-white p-6 rounded-lg shadow-md text-center"><p className="text-gray-500">No alerts have been recorded. The system is stable.</p></div>}
        </div>
    );
};
...
function App() {
    ...
    useEffect(() => {
        const fetchData = async () => { try { const [latest, historical, alertData, efficiency] = await Promise.all([ fetch(`${API_BASE_URL}/latest-data`), fetch(`${API_BASE_URL}/historical-data`), fetch(`${API_BASE_URL}/alerts`), fetch(`${API_BASE_URL}/efficiency-proof`) ]); if (latest.ok) setLatestData(await latest.json()); if (historical.ok) setHistoricalData(await historical.json()); if (efficiency.ok) setEfficiencyProof(await efficiency.json()); if (alertData.ok) { const newAlerts = await alertData.json(); setAlerts(newAlerts.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp))); newAlerts.forEach(alert => { if (alert.severity === 'CRITICAL' && !spokenAlerts.current.has(alert.timestamp) && 'speechSynthesis' in window) { const utterance = new SpeechSynthesisUtterance(`Critical Alert: ${alert.message}`); window.speechSynthesis.speak(utterance); spokenAlerts.current.add(alert.timestamp); } }); } } catch (e) { console.error("Fetch failed:", e); } finally { setLoading(false); } };
